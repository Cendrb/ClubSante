function setupHandlersForTimetableModification()
{
	...
	$(".timetable_modification_template").click(function(event)
	{
		var clicked = $(this);
	...
	$(".timetable_modification_weekday").click(function(event)
	{
		...
		$.ajax({
			type: "GET",
			url: "/exercise_modifications/new",
			data: { datetime: clicked.data("date") + " " + beginningHours + ":" + beginningMinutes + " UTC", calendar_id: clicked.data("calendar-id") },
			dataType: 'script',
			format: 'js',
			success: function(msg){
			}
